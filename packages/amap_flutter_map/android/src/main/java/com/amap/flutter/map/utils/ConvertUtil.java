...
// asset lookup helper (avoid relying on FlutterLoader.getInstance())
...
//        }
    ...
    private static String getLookupKeyForAsset(String asset, String packageName) {
        if (asset == null) return null;
        if (packageName == null || packageName.isEmpty()) return getLookupKeyForAsset(asset);
        return "flutter_assets/packages/" + packageName + "/" + asset;
    }
